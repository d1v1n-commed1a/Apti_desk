...
Apti desk는 ✨인적성 시험✨ 풀이와 복기를 한 번에 관리할 수 있도록 만든 단일 HTML 기반 학습 도구입니다.
OMR 입력, 정답 등록, 채점/재채점, 오답 정리, 과목별 문항 구성, 타이머, Google Sheets 백업 기능을 제공합니다.
모니터 우측 패널에 띄워두고 사용하는 것을 전제로 개발했으며, ebook 또는 PDF와 나란히 배치해 실제 시험과 유사한 환경에서 인적성 대비 학습을 할 수 있습니다.
...
- `v0.100 -> v0.110`
  Google Sheets 저장 시 같은 `시험명 + 회차` 조합이면 기존 행을 덮어쓰도록 정리했습니다.

- `v0.110 -> v0.120`
  모의고사 모드와 과목 구성 기능을 추가했습니다. 시험/회차별로 문항 수와 과목 구성을 설정하고 저장/복원할 수 있게 확장했습니다.

- `v0.120 -> v0.130` 📌
  사전 정답표 등록, 채점/재채점 흐름, 오답 및 미응답 문항 노란색 재입력, 과목별/총계 결과 요약, 재채점 시 빨간 정답 표시를 추가했습니다.
  `SK그룹 SKCT` 선택 시 `언어이해/자료해석/창의수리/언어추리/수열추리` 각 20문항 기본 구성이 자동으로 들어가며, 이후 사용자가 자유롭게 수정할 수 있습니다.
  문항 소요시간은 문항별 누적 방식으로 수정했고, 타이머 pause/resume 구간은 제외되도록 정리했습니다.
